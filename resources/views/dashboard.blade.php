            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <table class="min-w-full border border-gray-300 shadow-md rounded-lg overflow-hidden">
                            ...
                            <tbody class="divide-y divide-gray-300 bg-gray-100 text-black">
                                ...
                                <tr class="hover:bg-gray-100">
                                    ...
                                    <td class="px-4 py-2">{{ \Carbon\Carbon::parse($event->start_time)->format('Y-m-d') }}</td>
                                    <td class="px-4 py-2">{{ \Carbon\Carbon::parse($event->start_time)->format('H:i A') }} - {{ \Carbon\Carbon::parse($event->end_time)->format('H:i A') }}</td>
                                    <td class="px-4 py-2">
                    ...
                    {{ now()->lt($event->start_time) ? 'bg-blue-500' : (now()->between($event->start_time, $event->end_time) ? 'bg-green-500' : 'bg-red-500') }}">
                                    ...
                                    <td class="px-4 py-2">
                                        <a class="bg-blue-500 text-white py-2 px-4 rounded w-full block text-center hover:bg-blue-600" href="/edit-ticket/{{$event->id}}">Edit</a>
                                        <a class="bg-red-500 text-white py-2 px-4 rounded w-full block text-center hover:bg-red-600" href="/delete-ticket/{{$event->id}}">Delete</a>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
